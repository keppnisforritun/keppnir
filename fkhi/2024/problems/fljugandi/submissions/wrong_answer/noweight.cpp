...
struct bigpt3 {
    __int128 x, y, z;
    bigpt3(__int128 a, __int128 b, __int128 c) :
        x(a), y(b), z(c) { }
    bool iszero() {
        return x == 0 && y == 0 && z == 0;
    }
...
struct pt3 {
    ...
    __int128 dot(const pt3 &o) const {
        ll dx = x; dx *= o.x;
        ll dy = y; dy *= o.y;
        __int128 dz = z; dz *= o.z;
        return dx + dy + dz;
    }
    __int128 bigdot(const bigpt3 &o) const {
...
struct hullmaker {
    ...
    vi hull3(vector<pt3> &p) {
        ...
        for(int i = 3; i < n; ++i) {
            for(int F : { F1, F2 }) {
                __int128 Q = (p[i] - p[faces[F].a]).bigdot(faces[F].q);
                if(Q >= 0) {
...
bool touch(ll h, pt3 a, pt3 b, ll ra, ll rb) {
    ...
    __int128 ra2 = ra * ra - h * h, rb2 = rb * rb - h * h;
    __int128 dx = a.x - b.x, dy = a.y - b.y;
    __int128 nrm = dx * dx + dy * dy;
    __int128 rhs = nrm - ra2 - rb2;
...
int main() {
    ...
    while(hi > lo) {
        ...
        for(int i = 0; i < n; ++i) {
            ...
            pts.emplace_back(xs[i], ys[i], xs[i] * xs[i] + ys[i] * ys[i]);
            pts.back().i = i;
            if(cur_r[i] > h) {
