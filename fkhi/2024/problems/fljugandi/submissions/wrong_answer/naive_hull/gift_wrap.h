...
template <typename T>
std::vector<point2d_t<T>> get_points_in_plane(const std::vector<vec3_t<T>> &points, plane3_t<T> plane) {
    const auto forward{ plane.normal };
    const vec3_t<T> right{ -forward.y, forward.x, 0 };
    const vec3_t<T> up{ 0, -forward.z, forward.x };
    std::vector<point2d_t<T>> result;
    for (size_t i = 0; i < size(points); i++) {
        const auto& p{ points[i] };
        if (plane.contains(p)) {
            result.push_back({ i, { right % p, up % p } });
        }
    }
    return result;
}
...
hull_t gift_wrap(std::vector<vec3_t<T>> points) {
    ...
    while (!q.empty()) {
        ...
        const auto points_in_plane{ get_points_in_plane(points, plane) };
...
template <typename T>
hull_t convex_hull_3d(std::vector<vec3_t<T>> points) {
    ...
    std::set<face_t> processed;
    auto add_to_processed = [&processed](const std::vector<point2d_t<T>>& points) {
        for (std::size_t i = 0; i < size(points); i++) {
            for (std::size_t j = i + 1; j < size(points); j++) {
                for (std::size_t k = j + 1; k < size(points); k++) {
                    processed.emplace(std::vector<size_t>{points[i].first, points[j].first, points[k].first});
                }
            }
        }
    };
    for (std::size_t i = 0; i < n; i++) {
        ...
        for (std::size_t j = i + 1; j < n; j++) {
            ...
            for (std::size_t k = j + 1; k < n; k++) {
                const auto& R{ points[k] };
                if (processed.contains({i, j, k})) {
                    continue;
                }
                std::vector<std::size_t> positive, negative;
                for (std::size_t v = 0; v < n; v++) {
                    const auto& point{ points[v] };
                    const auto d{ point.unnormalized_signed_distance(P, Q, R) };
                    if (d > EPS<T>) {
                ...
                const auto points_in_plane{ get_points_in_plane(points, plane) };
                add_to_processed(points_in_plane);
                const auto hull{ jarvis_march(points_in_plane) };
                assert(hull.size() >= 3);
                if (positive.empty() || negative.empty()) {
                    for (std::size_t ind = 1; ind + 1 < std::size(hull); ind++) {
                        face_t face;
                        face.reserve(face.size());
                        face.push_back(hull[0].first);
                        face.push_back(hull[ind].first);
                        face.push_back(hull[ind+1].first);
                        if (negative.empty()) {
