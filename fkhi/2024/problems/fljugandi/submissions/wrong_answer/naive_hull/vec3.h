...
struct vec3_t {
    ...
    constexpr scalar_t unnormalized_signed_distance(const vec_t A, const vec_t B, const vec_t C) const {
    ...
    constexpr vec_t rotate(const vec_t normal) const {
        ...
        assert(abs(normal.length_squared() - 1) <= EPS<scalar_t>);
        return *this * normal;
    }
    ...
    constexpr bool is_zero() const {
        return abs(x) <= EPS<scalar_t> && abs(y) <= EPS<scalar_t> && abs(z) <= EPS<scalar_t>;
    }
