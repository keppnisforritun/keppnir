# Sortedcontainers from Grant Jenks
...
def ev_order(e1, e2):
    return xy_order(e1[2], e2[2])

def is_left(p0, p1, p2):
    return (p1 - p0).real * (p2 - p0).imag - (p2 - p0).real * (p1 - p0).imag

# event -> tuple (edge, type, point)

class event_queue:
    def __init__(self, p):
        self.ix = 0
        self.ne = 2 * len(p)
        self.dat = [None for i in range(self.ne)]
        for i in range(len(p)):
            t = 0 if xy_order(p[i], p[(i + 1) % len(p)]) < 0 else 1
            self.dat[2 * i] = (i, t, p[i])
            self.dat[2 * i + 1] = (i, 1 - t, p[(i + 1) % len(p)])
        self.dat.sort(key=cmp_to_key(ev_order))
    def next(self):
        if self.ix >= self.ne:
            return None
        self.ix += 1
        return self.dat[self.ix - 1]

# sl_seg -> tuple (edge, lp, rp)
# above, below not needed in this version

class sweep_line:
    def __init__(self, p):
        self.nv = len(p)
        self.pn = p
        self.tree = SortedSet([], key=itemgetter(0))
    def add(self, ev):
        v1, v2 = self.pn[ev[0]], self.pn[(ev[0] + 1) % self.nv]
        if xy_order(v1, v2) < 0:
            v1, v2 = v2, v1
        sl = (ev[0], v1, v2)
        self.tree.add(sl)
        return sl
    def find(self, ev):
        return self.tree[self.tree.bisect_key_left(ev[0])]
    def below(self, sl):
        if sl not in self.tree:
            return None
        if len(self.tree) < 2:
            return None
        i = self.tree.bisect_key_left(sl[0])
    ...
    def above(self, sl):
        ...
        if len(self.tree) < 2:
            return None
        i = self.tree.bisect_key_left(sl[0])
        return self.tree[(i + 1) % len(self.tree)]
    def intersect(self, s1, s2):
        ...
        if s2 is None:
            return False
        e1, e2 = s1[0], s2[0]
        if (e1 + 1) % self.nv == e2 or e1 == (e2 + 1) % self.nv:
            return False
        if abs((s1[1] - s1[2]).real) < EPS and abs((s2[1] - s2[2]).real) < EPS:
            a1, b1 = s1[1].imag, s1[2].imag
            if a1 > b1:
                a1, b1 = b1, a1
            a2, b2 = s2[1].imag, s2[2].imag
            if a2 > b2:
                a2, b2 = b2, a2
            return (a1 < b2) and (a2 > b1)
        lsign1 = is_left(s1[1], s1[2], s2[1])
        rsign1 = is_left(s1[1], s1[2], s2[2])
        lsign2 = is_left(s2[1], s2[2], s1[1])
        rsign2 = is_left(s2[1], s2[2], s1[2])
        if lsign2 * rsign2 > EPS:
            return False
        if lsign1 * rsign1 > EPS:
            return False
        return True
...
def simple(p):
    ...
    while True:
        ...
        if ev[1] == 0:
            ...
            above = sl.above(seg)
            below = sl.below(seg)
            if sl.intersect(seg, above):
                return False
            if sl.intersect(seg, below):
                return False
        else:
            seg = sl.find(ev)
            above = sl.above(seg)
            below = sl.below(seg)
            if above != below and sl.intersect(above, below):
                return False
            sl.tree.discard(seg)
    return True
...
def collinear(a, b, p, q):
    return abs(ccw(a, b, p)) < EPS and abs(ccw(a, b, q)) < EPS

def normalize(p, k = 1.0):
    if abs(p) == 0.0:
        return p
    return p / abs(p) * k
...
def push(a, b, c, d):
    v1 = normalize((b - a) * 1j, d)
    v2 = normalize((c - b) * 1j, d)
    return intersect(a + v1, b + v1, b + v2, c + v2)

def contract(p, d):
    return [push(p[i - 1], p[i], p[(i + 1) % len(p)], d) for i in range(len(p))]

if __name__ == "__main__":
    n = int(input())
    p = [complex(*map(float, sys.stdin.readline().strip().split())) for i in range(n)]
    slp = []
    for i in range(n):
        slp.append(cmath.phase(p[i] - p[i - 1]))
    slp.sort()
    choice = (slp[-1] + slp[0] + 2 * math.pi) / 2.0
    bst = abs(slp[-1] - slp[0] - 2 * math.pi)
    for i in range(1, n):
        if abs(slp[i] - slp[i - 1]) > bst:
            choice = (slp[i] + slp[i + 1]) / 2
            bst = abs(slp[i] - slp[i - 1])
    for i in range(n):
        p[i] *= cmath.rect(1.0, choice)
    mnind = 0
    for i in range(1, n):
        if p[i].imag < p[mnind].imag:
            mnind = i
        elif p[i].imag == p[mnind].imag and p[i].real > p[mnind].real:
            mnind = i
    l, r, a, poss = 0, 2e9, polygon_area(p), False
    while r - l > 1e-6 and (r - l) / (l + EPS) > 1e-6:
        m = (l + r) / 2.0
        q = contract(p, m)
        if q[(mnind + 1) % n].imag < q[mnind].imag - EPS:
            r = m
        elif q[mnind - 1].imag < q[mnind].imag - EPS:
            r = m
        elif not simple(q):
            r = m
        elif polygon_area(q) > a / 2.0:
            l = m
        else:
            r = m
            poss = True
